...
class DRDetector(DenseDetector):
    def forward(self, batched_inputs: List[Dict[str, Tensor]]):
        """
        Args:
            batched_inputs: a list, batched outputs of :class:`DatasetMapper` .
        ...
        features = self.backbone(images.tensor)
        features = [features[f] for f in self.head_in_features]
        if any([x.isnan().any() for x in features]):    print(sf_dict)
        assert not any([x.isnan().any() for x in features])
        predictions = self.head(features)
        ...
        if self.training:
            ...
            assert "instances" in batched_inputs[0], "Instance annotations are missing in training!"
            gt_instances = [x["instances"].to(self.device) for x in batched_inputs]
            return self.forward_training(images, features, predictions, gt_instances, sf_dict)
        else:
            results = self.forward_inference(images, features, predictions)
            if torch.jit.is_scripting():
                return results
