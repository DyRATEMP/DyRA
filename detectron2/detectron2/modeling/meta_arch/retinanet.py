...
from .dense_detector import DenseDetector, permute_to_N_HWA_K, DRDetector  # noqa

from ..resizer import DynamicResizer
...
class RetinaNet(DRDetector):
    ...
    def forward_training(self, images, features, predictions, gt_instances, sf_dicts=None):
        ...
        return self.losses(anchors, pred_logits, gt_labels, pred_anchor_deltas, gt_boxes, sf_dicts)

    def losses(self, anchors, pred_logits, gt_labels, pred_anchor_deltas, gt_boxes, sf_dicts=None):
        """
        Args:
        ...
        loc_val = (loss_box_reg.sum() / normalizer).item()
        if self.resizer is not None:
            resizer_dict = self.resizer.balance_loss(torch.stack(gt_boxes)[pos_mask], loss_box_reg)
            resizer_dict.update(sf_dicts)
            resizer_dict = dict({k:resizer_dict[k]*loc_val for k in resizer_dict})
        else:
            resizer_dict = {}
            
        return dict({"loss_cls": loss_cls / normalizer, 
                     "loss_box_reg": loss_box_reg.sum() / normalizer,}, **resizer_dict)
