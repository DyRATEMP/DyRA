...
class AutoResizer(nn.Module): 
    ...
    def __init__(self, *, 
        ...
        self.mapping_ratio = nn.Parameter((self.grid_sizes[1::2]**2).mean() / self.coco_bnd.mean())
    ...
    def from_config(cls, cfg):
        image_encoder = build_backbone(cfg, is_encoder=True)
        
        ## for class_num
        return {
            "net":SimplePredictor(in_chan=15,
                                  device=cfg.MODEL.DEVICE,
                                  num_mlayer=cfg.MODEL.ENCODER.RES2_OUT_CHANNELS*4),
            "image_encoder": image_encoder,
            "out_layer": cfg.MODEL.ENCODER.OUT_FEATURES,
    ...
    def loss(self, batched_inputs, sc_sf):
        ...
        return {"loss_scale_reg": torch.stack(sc_losses, dim=0).mean()}
    
    def balance_loss(self, anchors: List[Union[Boxes, torch.Tensor]],
                     box2box_transform: Box2BoxTransform,
                     boxes: List[torch.Tensor],
                     fg_mask: torch.Tensor, loc_loss: torch.Tensor):
        
        if anchors is not None:
            if isinstance(anchors[0], Boxes):
                anchors = type(anchors[0]).cat(anchors).tensor  # (R, 4)
            elif isinstance(anchors[0][0], Boxes):
                anchors = [type(anchor[0]).cat(anchor).tensor for anchor in anchors] # (N, R, 4)
            else:
                anchors = cat(anchors)
            
            box_deltas = [box2box_transform.get_deltas(anchors[i], k)
                          if isinstance(anchors, list) else box2box_transform.get_deltas(anchors, k)
                          for i, k in enumerate(boxes)]
            box_deltas = torch.stack(box_deltas)[fg_mask]  # (N, R, 4)
        else:
            box_deltas = boxes
        ...
        per_size_losses = torch.stack([loc_loss[torch.where(box_sizes<ac_size_mean)].mean(), 
                                       loc_loss[torch.where(box_sizes>=ac_size_mean)].mean()])
        loc_weight = inv_mean_max(per_size_losses.nan_to_num(nan=0.1)) * per_size_losses.numel()
        loc_weight += -loc_weight.min() + 1
    ...
    # def get_cor_val(self, x, target_range, xrange=torch.tensor([6, -6])):
    #     x = (-(x.pow(-1) - 1).log()).clamp(min=xrange.min(), max=xrange.max())
    #     x *= (target_range.max() - target_range.min()) / (xrange[1] - xrange[0]) 
    #     x += target_range.mean()
    #     return x