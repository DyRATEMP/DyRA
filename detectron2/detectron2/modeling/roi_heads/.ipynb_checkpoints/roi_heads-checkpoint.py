    ...
    def forward(
        ...
        if self.training:
            ...
            losses = self.box_predictor.losses(predictions, proposals, 
                                               resizer=resizer, reduction=reduction)
            if self.mask_on:
...
class StandardROIHeads(ROIHeads):
    ...
    def forward(
        ...
        if self.training:
            losses = self._forward_box(features, proposals, resizer=resizer, reduction=reduction)
    ...
    def _forward_box(self, features: Dict[str, torch.Tensor], proposals: List[Instances],
        ...
        if self.training:
            if self.mask_on: resizer, reduction = None, "sum"
            losses = self.box_predictor.losses(predictions, proposals, 
                                               resizer=resizer, reduction=reduction)
