    ...
    def __init__(self, in_chan, out_chan=1, gain=2, grid_len=3, num_mlayer=1024, device='cuda'):
        ...
        self.enc_layers = nn.Sequential(
            nn.TransformerEncoder(
                nn.TransformerEncoderLayer(d_model=in_chan, nhead=int(in_chan/grid_len)), num_layers=3),
        )
        
        self.out_layers = nn.Sequential(nn.Sigmoid(), Multiply(gain))
        self.fc_layer = nn.Sequential(nn.Linear(in_chan, 1),)
...
#         net.train()
    ...
    def forward(self, x):
        x = self.cvt_layers(x)
        x = self.enc_layers(x)
        x = self.out_layers(self.fc_layer(x))
        return x