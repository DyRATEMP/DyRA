...
class FastRCNNOutputLayers(nn.Module):
    ...
    def losses(self, predictions, proposals, reduction="sum", resizer=None):
        """
        Args:
        ...
        if resizer is not None:    reduction="none"
        loss_box, infos = self.box_reg_loss(proposal_boxes, gt_boxes, proposal_deltas, 
                                            gt_classes, reduction=reduction, use_norm=False)
        losses = {
            "loss_cls": loss_cls,
            "loss_box_reg": loss_box.sum() / infos.get("normalizer"),
        }
        
        if resizer is not None:
            balance_dict = resizer.balance_loss(torch.stack([gt_boxes[infos.get("inds")]]), loss_box)
            loc_val = losses.get("loss_box_reg").item()
            balance_dict = dict({k:balance_dict[k]*loc_val for k in balance_dict})
        else:
            balance_dict = {}
        
        losses = {k: v * self.loss_weight.get(k, 1.0) for k, v in losses.items()}
        losses.update(balance_dict)
        return losses
    ...
    def box_reg_loss(self, proposal_boxes, gt_boxes, pred_deltas, gt_classes, 
        ...
        if use_norm:
            ...
            return loss_box_reg, {"normalizer": max(gt_classes.numel(), 1.0), 
                                  "inds": fg_inds, "proposals": proposal_boxes}
