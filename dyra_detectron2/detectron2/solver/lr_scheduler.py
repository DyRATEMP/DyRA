...
class ConstCosineParamScheduler(CosineParamScheduler):
    def __init__(self, start_value: float, end_value: float, ratio=1/3) -> None:
        # quater cosine and constant learning rate combination
    ...
    def __call__(self, where: float) -> float:
        return self._end_value + 0.5 * (self._start_value - self._end_value) * (
            min(2 * math.cos(4 * math.pi * where * self._ratio - math.pi * self._ratio), 0) + 2
        )
