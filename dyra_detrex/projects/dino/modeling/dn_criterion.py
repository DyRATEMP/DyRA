...
class DINOCriterion(TwoStageCriterion):
    ...
    def forward(self, outputs, targets, dn_metas=None, resizer=None, gt_sizes=None):
    ...
    def compute_dn_loss(self, dn_metas, targets, aux_num, num_boxes, 
        ...
        if dn_metas and "output_known_lbs_bboxes" in dn_metas:
            ...
            if resizer is not None: 
                gt_boxes = torch.cat([t[i] for t, (_, i) in zip(gt_sizes, dn_idx)], dim=0)

                keys = [key for key in l_dict if "loc" in key or "iou" in key or "box" in key]
                bal_losses = [resizer.balance_loss(gt_boxes, l_dict[k]*num_boxes).pop("loss_bal")*l_dict[k].sum().item() 
                              for k in keys]

                losses.update({"loss_bal": -((-torch.stack(bal_losses)).exp()).prod().log()})
                loc_dict = {k:l_dict[k].sum() for k in l_dict if k in keys}
                l_dict.update(loc_dict)
            
            l_dict = {k + "_dn": v.mean() for k, v in l_dict.items()} 
