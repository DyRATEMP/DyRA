...
class HDeformableDETR(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            gt_sizes = [x["instances"].gt_boxes.tensor.to(self.device) for x in batched_inputs]
            ...
            if self.k_one2many > 0:
                ...
                loss_dict = self.criterion(output, targets, resizer=self.resizer, gt_sizes=gt_sizes)
            weight_dict = self.criterion.weight_dict
            
            if self.resizer is not None:
                loc_val = [loss_dict[key].sum() for key in loss_dict if "many" not in key and ("box" in key or "iou" in key)]
                sf_dict = {k: -((-torch.stack(loc_val)).exp().prod().log()).item() * sf_dict[k]
                           for k in sf_dict}
    ...
    def train_hybrid(self, outputs, targets, k_one2many, criterion, lambda_one2many, resizer=None, gt_sizes=None):
        # one-to-one-loss
        loss_dict = criterion(outputs, targets, resizer=resizer, gt_sizes=gt_sizes)
